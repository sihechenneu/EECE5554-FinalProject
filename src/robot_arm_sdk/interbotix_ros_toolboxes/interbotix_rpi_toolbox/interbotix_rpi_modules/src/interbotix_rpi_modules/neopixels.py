...
from interbotix_rpi_modules_msgs.msg import PixelCommands


class InterbotixRpiPixelInterface(object):
    def __init__(self, robot_name, node=None):
        self._node = node
        self._owns_node = False
        if self._node is None:
            rclpy.init()
            self._node = Node("rpi_pixel_interface")
            self._owns_node = True
        self.pub_pixels = self._node.create_publisher(
            PixelCommands, "/" + robot_name + "/commands/pixels", 5
        )
        time.sleep(1)
