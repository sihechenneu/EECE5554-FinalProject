...
/// @param node_handle - ROS 2 Node
InterbotixRobotXS::InterbotixRobotXS(rclcpp::Node* node_handle, bool& success)
    : node_(node_handle)
{
  ...
  {
    success = false;
    RCLCPP_FATAL(node_->get_logger(), "[xs_sdk] Failed to find all motors specified in the motor_config file after three attempts. Shutting down...");
    return;
  }
  ...
  {
    success = false;
    RCLCPP_FATAL(node_->get_logger(), "[xs_sdk] Failed to write configurations to all motors. Shutting down...");
    return;
  }
  ...
  RCLCPP_INFO(node_->get_logger(), "[xs_sdk] Interbotix 'xs_sdk' node is up!");
}
...
void InterbotixRobotXS::robot_set_operating_modes(std::string const& cmd_type, std::string const& name, std::string const& mode, std::string const& profile_type, int32_t profile_velocity, int32_t profile_acceleration)
{
  ...
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    ...
    RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The operating mode for the '%s' joint was changed to %s.", name.c_str(), mode.c_str());
  }
  else if ((cmd_type == "group" && group_map.count(name) == 0) || (cmd_type == "single" && motor_map.count(name) == 0))
    RCLCPP_WARN(node_->get_logger(),"[xs_sdk] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid command for argument 'cmd_type' while setting operating mode.");
}
...
void InterbotixRobotXS::robot_set_joint_operating_mode(std::string const& name, std::string const& mode, std::string const& profile_type, int32_t profile_velocity, int32_t profile_acceleration)
{
  for (auto const& joint_name:sister_map[name])
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, torqued off.", motor_map[joint_name].motor_id);
  }
  ...
  for (auto const& motor_name:shadow_map[name])
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, read Drive Mode %d.", motor_map[motor_name].motor_id, drive_mode);

    if (drive_mode <= 1 && profile_type == "time")
    {
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Drive_Mode", drive_mode + 4);
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, write Drive Mode %d.", motor_map[motor_name].motor_id, drive_mode + 4);
    }
    ...
    if (mode == "position" || mode == "linear_position")
    {
      ...
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, set poscontrolmode, pv=%i, pa=%i.", motor_map[motor_name].motor_id, profile_velocity, profile_acceleration);
    }
    else if (mode == "ext_position")
    {
      ...
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Profile_Velocity", profile_velocity);
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Profile_Acceleration", profile_acceleration);
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, set extposcontrolmode, pv=%i, pa=%i.", motor_map[motor_name].motor_id, profile_velocity, profile_acceleration);
    ...
    else if (mode == "velocity")
    {
      ...
      dxl_wb.itemWrite(motor_map[motor_name].motor_id, "Profile_Acceleration", profile_acceleration);
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, set velcontrolmode, pa=%i.", motor_map[motor_name].motor_id, profile_acceleration);
    }
    else if (mode == "pwm")
    {
      dxl_wb.setPWMControlMode(motor_map[motor_name].motor_id);
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, set pwmcontrolmode.", motor_map[motor_name].motor_id);
    }

    else if (mode == "current")
    {
      dxl_wb.setCurrentControlMode(motor_map[motor_name].motor_id);
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, set currentcontrolmode", motor_map[motor_name].motor_id);
    }
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid command for argument 'mode' while setting the operating mode for the %s motor.", motor_name.c_str());
      continue;
    }
  ...
  for (auto const& joint_name:sister_map[name])
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_joint_operating_mode] ID: %d, torqued on.", motor_map[joint_name].motor_id);
  }
...
void InterbotixRobotXS::robot_torque_enable(std::string const& cmd_type, std::string const& name, bool const& enable)
{
  if (cmd_type == "group" && group_map.count(name) > 0)
  {
    ...
    if (enable) RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' group was torqued on.", name.c_str());
    else RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' group was torqued off.", name.c_str());
  }
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    dxl_wb.torque(motor_map[name].motor_id, enable);
    if (enable) RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' joint was torqued on.", name.c_str());
    else RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' joint was torqued off.", name.c_str());
  }
  else if ((cmd_type == "group" && group_map.count(name) == 0) || (cmd_type == "single" && motor_map.count(name) == 0))
    RCLCPP_WARN(node_->get_logger(),"[xs_sdk] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid command for argument 'cmd_type' while torquing joints.");
}
...
void InterbotixRobotXS::robot_reboot_motors(std::string const& cmd_type, std::string const& name, bool const& enable, bool const& smart_reboot)
{
  ...
  if (cmd_type == "group" && group_map.count(name) > 0)
  {
    for (auto const& joint_name:group_map[name].joint_names)
    {
      ...
      RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' joint was rebooted.", joint_name.c_str());
      if (enable) joints_to_torque.push_back(joint_name);
    }
    if (!smart_reboot)
      RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' group was rebooted.", name.c_str());
  }
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    dxl_wb.reboot(motor_map[name].motor_id);
    RCLCPP_INFO(node_->get_logger(),"[xs_sdk] The '%s' joint was rebooted.", name.c_str());
    if (enable) joints_to_torque.push_back(name);
  }
  else if ((cmd_type == "group" && group_map.count(name) == 0) || (cmd_type == "single" && motor_map.count(name) == 0))
    RCLCPP_WARN(node_->get_logger(),"[xs_sdk] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid command for argument 'cmd_type' while rebooting motors.");
...
void InterbotixRobotXS::robot_write_commands(std::string const& name, std::vector<float> commands)
{
  ...
  {
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Number of commands (%ld) does not match the number of joints in group '%s' (%d). Will not execute.", commands.size(), name.c_str(), group_map[name].joint_num);
    return;
  }
  ...
  if (mode == "position" || mode == "ext_position" || mode == "current_based_position" || mode == "linear_position")
  {
    for (size_t i{0}; i < commands.size(); i++)
    {
      ...
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_write_commands] ID: %d, writing %s command %d.", group_map[name].joint_ids.at(i), mode.c_str(), dynamixel_commands[i]);
    }
    dxl_wb.syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, group_map[name].joint_ids.data(), group_map[name].joint_num, dynamixel_commands.data(), 1);
  }
  else if (mode == "velocity")
  {
    for (size_t i{0}; i < commands.size(); i++)
    ...
    dxl_wb.syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_VELOCITY, group_map[name].joint_ids.data(), group_map[name].joint_num, dynamixel_commands.data(), 1);
  }
  else if (mode == "current")
  {
    for (size_t i{0}; i < commands.size(); i++)
    ...
    dxl_wb.syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_CURRENT, group_map[name].joint_ids.data(), group_map[name].joint_num, dynamixel_commands.data(), 1);
  }
  else if (mode == "pwm")
  {
    for (size_t i{0}; i < commands.size(); i++)
    ...
    dxl_wb.syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_PWM, group_map[name].joint_ids.data(), group_map[name].joint_num, dynamixel_commands.data(), 1);
  }
  else
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid command for argument 'mode' while commanding joint group.");
}
...
void InterbotixRobotXS::robot_write_joint_command(std::string const& name, float command)
{
  ...
  if (mode == "position" || mode == "ext_position" || mode == "current_based_position" || mode == "linear_position")
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_write_joint_command] ID: %d, writing %s command %f.", motor_map[name].motor_id, mode.c_str(), command);
  ...
  else if (mode == "pwm")
  {
    ...
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid command for argument 'mode' while commanding joint.");
}
...
void InterbotixRobotXS::robot_set_motor_pid_gains(std::string const& cmd_type, std::string const& name, std::vector<int32_t> const& gains)
{
  ...
  for (auto const& name : names)
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains] ID: %d, writing gains:", motor_map[name].motor_id);
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         Pos_P: %i", gains.at(0));
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         Pos_I: %i", gains.at(1));
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         Pos_D: %i", gains.at(2));
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         FF_1: %i", gains.at(3));
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         FF_2: %i", gains.at(4));
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         Vel_P: %i", gains.at(5));
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_pid_gains]         Vel_I: %i", gains.at(6));
...
void InterbotixRobotXS::robot_set_motor_registers(std::string const& cmd_type, std::string const& name, std::string const& reg, int32_t const& value)
{
  ...
  for (auto const& name : names)
  {
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_set_motor_registers] ID: %d, writing reg: %s, value: %d.", motor_map[name].motor_id, reg.c_str(), value);
...
void InterbotixRobotXS::robot_get_motor_registers(std::string const& cmd_type, std::string const& name, std::string const& reg, std::vector<int32_t> &values)
{
  ...
  if (goal_reg == NULL)
  {
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get '%s' Item Info. Did you spell the register name correctly?", reg.c_str());
    return;
  }
  ...
  for (auto const& name : names)
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] %s", log);
      return;
    }
    else
    {
      RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_get_motor_registers] ID: %d, reading reg: %s, value: %d.", motor_map[name].motor_id, reg.c_str(), value);
    }
...
void InterbotixRobotXS::robot_get_joint_states(std::string const& name, std::vector<float> *positions, std::vector<float> *velocities, std::vector<float> *effort)
{
  for (auto const& joint_name : group_map[name].joint_names)
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_get_joint_states] ID: %ld, got joint state.", js_index_map[joint_name]);
  }
}
...
void InterbotixRobotXS::robot_get_joint_state(std::string const& name, float *position, float *velocity, float *effort)
{
  ...
  RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_get_joint_state] ID: %ld, got joint state.", js_index_map[name]);
}
...
bool InterbotixRobotXS::robot_get_motor_configs(void)
{
  ...
  node_->declare_parameter<std::string>("motor_configs", "");
  node_->get_parameter("motor_configs", motor_configs_file);
  try
  {
  ...
  {
    RCLCPP_FATAL(node_->get_logger(),"[xs_sdk] Motor Config file at '%s' was not found or has a bad format. Shutting down...", motor_configs_file.c_str());
    RCLCPP_FATAL(node_->get_logger(),"[xs_sdk] YAML Error: '%s'", error.what());
    return false;
  }
  if (motor_configs.IsNull())
  {
    RCLCPP_FATAL(node_->get_logger(),"[xs_sdk] Motor Config file at '%s' was not found. Shutting down...", motor_configs_file.c_str());
    return false;
  }

  node_->declare_parameter<std::string>("mode_configs", "");
  node_->get_parameter("mode_configs", mode_configs_file);
  try
  {
    mode_configs = YAML::LoadFile(mode_configs_file.c_str());
    RCLCPP_INFO(node_->get_logger(),"[xs_sdk] Loaded mode configs from '%s'.", mode_configs_file.c_str());
  }
  catch (YAML::BadFile &error)
  {
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Motor Config file at '%s' was not found or has a bad format. Shutting down...", mode_configs_file.c_str());
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] YAML Error: '%s'", error.what());
    return false;
  }
  if (mode_configs.IsNull())
    RCLCPP_INFO(node_->get_logger(),"[xs_sdk] Mode Config file is empty. Will use defaults.");

  port = motor_configs["port"].as<std::string>(PORT);
  if (mode_configs["port"])
    port = mode_configs["port"].as<std::string>(PORT);
  node_->declare_parameter<std::string>("port", "");
  std::string port_param;
  if (node_->get_parameter("port", port_param) && !port_param.empty())
    port = port_param;
  ...
  if (read_failure_behavior < ReadFailureBehavior::PUB_DXL_WB || read_failure_behavior > ReadFailureBehavior::PUB_NAN)
  {
    ...
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid option %d provided to joint_state_publisher.read_failure_behavior. Will default to option 0.", read_failure_behavior);
    read_failure_behavior = ReadFailureBehavior::PUB_DXL_WB;
  }
  RCLCPP_DEBUG(node_->get_logger(),"[xs_sdk::robot_get_motor_configs] read_failure_behavior set to %d.", read_failure_behavior);

  RCLCPP_INFO(node_->get_logger(),"[xs_sdk] Loaded motor configs from '%s'.", motor_configs_file.c_str());
  return true;
}
...
bool InterbotixRobotXS::robot_init_port(void)
{
  ...
  {
    RCLCPP_FATAL(node_->get_logger(),"[xs_sdk] Failed to open port at %s. Shutting down...", port.c_str());
    return false;
  }
  return true;
}
...
{
  node_->declare_parameter<std::string>("load_configs", "true");
  std::string load_configs_str;
  node_->get_parameter("load_configs", load_configs_str);
  load_configs = (load_configs_str == "true" || load_configs_str == "1" ||
                 load_configs_str == "True" || load_configs_str == "TRUE");
  if (load_configs)
  {
    RCLCPP_INFO(node_->get_logger(),
...
void InterbotixRobotXS::robot_init_controlItems(void)
{
  ...
  if (!goal_position)
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get 'Goal_Position' Item Info");
  ...
  if (!goal_velocity)
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get 'Goal_Velocity' or 'Moving_Speed' Item Info");
  ...
  if (!goal_pwm)
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get 'Goal_PWM' Item Info");

  const ControlItem *present_position = dxl_wb.getItemInfo(motor_id, "Present_Position");
  if (!present_position)
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get 'Present_Position' Item Info");
  ...
  if (!present_velocity)
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get 'Present_Velocity' or 'Present_Speed' Item Info");
  ...
  if (!present_current)
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Could not get 'Present_Current' or 'Present_Load' Item Info");
...
void InterbotixRobotXS::robot_init_SDK_handlers(void)
{
  if (!dxl_wb.addSyncWriteHandler(control_items["Goal_Position"]->address, control_items["Goal_Position"]->data_length))
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed to add SyncWriteHandler for Goal_Position.");

  if (!dxl_wb.addSyncWriteHandler(control_items["Goal_Velocity"]->address, control_items["Goal_Velocity"]->data_length))
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed to add SyncWriteHandler for Goal_Velocity.");
  ...
  if (control_items["Goal_Current"])
  {
    if (!dxl_wb.addSyncWriteHandler(control_items["Goal_Current"]->address, control_items["Goal_Current"]->data_length))
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed to add SyncWriteHandler for Goal_Current.");
  }
  else
    RCLCPP_INFO(node_->get_logger(),"[xs_sdk] SyncWriteHandler for Goal_Current not added as it's not supported.");

  if (!dxl_wb.addSyncWriteHandler(control_items["Goal_PWM"]->address, control_items["Goal_PWM"]->data_length))
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed to add SyncWriteHandler for Goal_PWM.");
  ...
  if (dxl_wb.getProtocolVersion() == 2.0f)
  {
    ...
    */
    ...
    if (!dxl_wb.addSyncReadHandler(start_address, read_length))
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed to add SyncReadHandler");
  }
}
...
{
  if (pub_states)
    pub_joint_states_ = node_->create_publisher<sensor_msgs::msg::JointState>(js_topic, 1);
}

/// @brief Initialize ROS Subscribers
void InterbotixRobotXS::robot_init_subscribers(void)
{
  sub_command_group_ = node_->create_subscription<interbotix_xs_msgs::msg::JointGroupCommand>(
    "commands/joint_group", 5, std::bind(&InterbotixRobotXS::robot_sub_command_group, this, std::placeholders::_1));
  sub_command_single_ = node_->create_subscription<interbotix_xs_msgs::msg::JointSingleCommand>(
    "commands/joint_single", 5, std::bind(&InterbotixRobotXS::robot_sub_command_single, this, std::placeholders::_1));
  sub_command_traj_ = node_->create_subscription<interbotix_xs_msgs::msg::JointTrajectoryCommand>(
    "commands/joint_trajectory", 5, std::bind(&InterbotixRobotXS::robot_sub_command_traj, this, std::placeholders::_1));
}

/// @brief Initialize ROS Services
void InterbotixRobotXS::robot_init_services(void)
{
  srv_torque_enable_ = node_->create_service<interbotix_xs_msgs::srv::TorqueEnable>("torque_enable", std::bind(&InterbotixRobotXS::robot_srv_torque_enable, this, std::placeholders::_1, std::placeholders::_2));
  srv_reboot_motors_ = node_->create_service<interbotix_xs_msgs::srv::Reboot>("reboot_motors", std::bind(&InterbotixRobotXS::robot_srv_reboot_motors, this, std::placeholders::_1, std::placeholders::_2));
  srv_get_robot_info_ = node_->create_service<interbotix_xs_msgs::srv::RobotInfo>("get_robot_info", std::bind(&InterbotixRobotXS::robot_srv_get_robot_info, this, std::placeholders::_1, std::placeholders::_2));
  srv_operating_modes_ = node_->create_service<interbotix_xs_msgs::srv::OperatingModes>("set_operating_modes", std::bind(&InterbotixRobotXS::robot_srv_set_operating_modes, this, std::placeholders::_1, std::placeholders::_2));
  srv_motor_gains_ = node_->create_service<interbotix_xs_msgs::srv::MotorGains>("set_motor_pid_gains", std::bind(&InterbotixRobotXS::robot_srv_set_motor_pid_gains, this, std::placeholders::_1, std::placeholders::_2));
  srv_set_registers_ = node_->create_service<interbotix_xs_msgs::srv::RegisterValues>("set_motor_registers", std::bind(&InterbotixRobotXS::robot_srv_set_motor_registers, this, std::placeholders::_1, std::placeholders::_2));
  srv_get_registers_ = node_->create_service<interbotix_xs_msgs::srv::RegisterValues>("get_motor_registers", std::bind(&InterbotixRobotXS::robot_srv_get_motor_registers, this, std::placeholders::_1, std::placeholders::_2));
}
...
void InterbotixRobotXS::robot_init_timers(void)
{
  ...
  if (timer_hz != 0)
    tmr_joint_states_ = node_->create_wall_timer(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(1.0 / timer_hz)), std::bind(&InterbotixRobotXS::robot_update_joint_states, this));
  tmr_joint_traj_ = node_->create_wall_timer(std::chrono::milliseconds(0), std::bind(&InterbotixRobotXS::robot_execute_trajectory, this));
  tmr_joint_traj_->cancel();
}
...
void InterbotixRobotXS::robot_wait_for_joint_states(void)
{
  ...
  while (rclcpp::ok() && joint_states.name.empty())
  {
    RCLCPP_DEBUG(node_->get_logger(), "[xs_sdk::robot_wait_for_joint_states] Waiting for joint states...");
    exec.spin_some(std::chrono::milliseconds(100));
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
  }
  RCLCPP_DEBUG(node_->get_logger(), "[xs_sdk::robot_wait_for_joint_states] Joint states found. Continuing.");
}

/// @brief ROS Subscriber callback function to command a group of joints
void InterbotixRobotXS::robot_sub_command_group(const interbotix_xs_msgs::msg::JointGroupCommand::SharedPtr msg)
{
  robot_write_commands(msg->name, msg->cmd);
}

/// @brief ROS Subscriber callback function to command a single joint
void InterbotixRobotXS::robot_sub_command_single(const interbotix_xs_msgs::msg::JointSingleCommand::SharedPtr msg)
{
...
void InterbotixRobotXS::robot_sub_command_traj(const interbotix_xs_msgs::msg::JointTrajectoryCommand::SharedPtr msg)
{
  if (execute_joint_traj)
  {
    RCLCPP_WARN(node_->get_logger(), "[xs_sdk] Trajectory rejected since joints are still moving.");
    return;
  }
  if (msg->traj.points.size() < 2)
  {
    RCLCPP_WARN(node_->get_logger(), "[xs_sdk] Trajectory has fewer than 2 points. Aborting...");
    return;
  }
  ...
  if (timer_hz != 0 && msg->traj.points[0].positions.size() == joint_names.size())
  {
    for (size_t i{0}; i < joint_names.size(); i++)
    {
      ...
      {
        RCLCPP_WARN(node_->get_logger(), "[xs_sdk] The %s joint is not at the correct initial state.", joint_names.at(i).c_str());
        RCLCPP_WARN(node_->get_logger(), "[xs_sdk] Expected state: %.2f, Actual State: %.2f.", expected_state, actual_state);
      }
    }
  }
  joint_traj_cmd = *msg;
...
bool InterbotixRobotXS::robot_srv_torque_enable(const std::shared_ptr<interbotix_xs_msgs::srv::TorqueEnable::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::TorqueEnable::Response>)
{
...
bool InterbotixRobotXS::robot_srv_reboot_motors(const std::shared_ptr<interbotix_xs_msgs::srv::Reboot::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::Reboot::Response>)
{
  if (!robot_srv_validate(req->cmd_type, req->name))
    return false;
...
/// @details - refer to the service definition for details
bool InterbotixRobotXS::robot_srv_get_robot_info(const std::shared_ptr<interbotix_xs_msgs::srv::RobotInfo::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::RobotInfo::Response> res)
{
  if (!robot_srv_validate(req->cmd_type, req->name))
    return false;
...
/// @details - refer to the service definition for details
bool InterbotixRobotXS::robot_srv_set_operating_modes(const std::shared_ptr<interbotix_xs_msgs::srv::OperatingModes::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::OperatingModes::Response>)
{
  if (!robot_srv_validate(req->cmd_type, req->name))
    return false;
...
bool InterbotixRobotXS::robot_srv_set_motor_pid_gains(const std::shared_ptr<interbotix_xs_msgs::srv::MotorGains::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::MotorGains::Response>)
{
  if (!robot_srv_validate(req->cmd_type, req->name))
    return false;
...
bool InterbotixRobotXS::robot_srv_set_motor_registers(const std::shared_ptr<interbotix_xs_msgs::srv::RegisterValues::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::RegisterValues::Response>)
{
  if (!robot_srv_validate(req->cmd_type, req->name))
    return false;
...
bool InterbotixRobotXS::robot_srv_get_motor_registers(const std::shared_ptr<interbotix_xs_msgs::srv::RegisterValues::Request> req, std::shared_ptr<interbotix_xs_msgs::srv::RegisterValues::Response> res)
{
  if (!robot_srv_validate(req->cmd_type, req->name))
    return false;
...
/// @brief One-Shot Timer callback to step through a commanded joint trajectory
void InterbotixRobotXS::robot_execute_trajectory()
{
  ...
  double t_prev = joint_traj_cmd.traj.points[cntr - 1].time_from_start.sec + 1e-9 * joint_traj_cmd.traj.points[cntr - 1].time_from_start.nanosec;
  double t_curr = joint_traj_cmd.traj.points[cntr].time_from_start.sec + 1e-9 * joint_traj_cmd.traj.points[cntr].time_from_start.nanosec;
  double period_sec = t_curr - t_prev;
  if (period_sec > 0)
  {
    if (tmr_joint_traj_)
      tmr_joint_traj_->cancel();
    auto period_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(period_sec));
    tmr_joint_traj_ = node_->create_wall_timer(period_ns, [this]() { robot_execute_trajectory(); });
  }
...
/// @brief Timer callback that reads current states from all the motors and publishes them to the joint_states topic
void InterbotixRobotXS::robot_update_joint_states()
{
  ...
  if (dxl_wb.getProtocolVersion() == 2.0f)
  {
    ...
    if (!dxl_wb.syncRead(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                          ...
                          &log))
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed syncRead: %s", log);
      read_failed = true;
    }
    ...
    if (!dxl_wb.getSyncReadData(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                                ...
                                &log))
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed getSyncReadData for Present_Current: %s", log);
      read_failed = true;
    }
    ...
    if (!dxl_wb.getSyncReadData(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                                ...
                                &log))
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed getSyncReadData for Present_Velocity: %s", log);
      read_failed = true;
    }
    ...
    if (!dxl_wb.getSyncReadData(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                                ...
                                &log))
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed getSyncReadData for Present_Position: %s", log);
      read_failed = true;
    }
  ...
  else if(dxl_wb.getProtocolVersion() == 1.0f)
  {
    ...
    for (auto const& id : all_ptr->joint_ids)
    {
      if (!dxl_wb.readRegister(id,
                                ...
                                &log))
      {
        RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed readRegister for joint states: %s", log);
        read_failed = true;
      }
  ...
  if (read_failed)
  {
    ...
    for (const auto id : all_ptr->joint_ids)
    {
      ...
      if (!dxl_wb.itemRead(id, "ID", &value))
      {
        ...
        RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Failed to read from DYNAMIXEL ID: %d", id);
      }
    }
  ...
  if (pub_states && pub_joint_states_)
...
bool InterbotixRobotXS::robot_srv_validate(const std::string &cmd_type, std::string &name)
{
  if (cmd_type == "group") // if group command...
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid service call. Group '%s' does not exist. Was it added to the motor config file?", name.c_str());
      return false;
    }
  }
  else if (cmd_type == "single") // if single joint command...
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid service call. Joint '%s' does not exist. Was it added to the motor config file?", name.c_str());
      return false;
    }
  }
  else // if command type is invalid, error and return false
  {
    RCLCPP_ERROR(node_->get_logger(),"[xs_sdk] Invalid service call. cmd_type '%s'. Choices are 'group' or 'single'.", cmd_type.c_str());
    return false;
  }
}
