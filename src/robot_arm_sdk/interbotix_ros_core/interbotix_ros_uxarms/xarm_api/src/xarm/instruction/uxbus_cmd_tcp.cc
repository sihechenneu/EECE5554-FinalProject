...
int UxbusCmdTcp::send_pend(int funcode, int num, int timeout, unsigned char *ret_data) {
  int i;
  int ret;
  std::vector<unsigned char> rx_data(arm_port_->que_maxlen_, 0);
  int times = timeout;
  while (times) {
    times -= 1;
    ret = arm_port_->read_frame(rx_data.data());
    if (ret != -1) {
      ret = check_xbus_prot(rx_data.data(), funcode);
...
int UxbusCmdTcp::send_xbus(int funcode, unsigned char *datas, int num) {
  int len = num + 7;
  std::vector<unsigned char> send_data(static_cast<size_t>(len));
  bin16_to_8(bus_flag_, &send_data[0]);
  bin16_to_8(prot_flag_, &send_data[2]);
  bin16_to_8(num + 1, &send_data[4]);
  send_data[6] = static_cast<unsigned char>(funcode);

  for (int i = 0; i < num; i++) { send_data[7 + i] = datas[i]; }
  arm_port_->flush();
  // print_hex("send:", send_data.data(), num + 7);
